...
class TestSiteDict(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_prefix = 'https://fake.fake/nwis/'
        self.mock_url = "https://fake.fake/nwis/site/?format=rdb&sites=05420680,05463500&siteStatus=all"
        ...
        self.mock_good_web_data = ("#\n"
                                   "#\n"
                                   "# US Geological Survey\n"
                                   "# retrieved: 2017-06-29 17:32:21 -04:00	(vaas01)\n"
                                   "#\n"
    ...
    def test_good_data(self):
        with requests_mock.Mocker() as m:
            m.get(self.mock_url, text=self.mock_good_web_data)
            self.assertEqual(site_dict(self.sites, self.mock_prefix), self.good_return)

    def test_missing_fields(self):
        with requests_mock.Mocker() as m:
            m.get(self.mock_url, text=self.mock_missing_huc)
            self.assertEqual(site_dict(self.sites, self.mock_prefix), self.missing_huc_return)

    def test_bad_status_code(self):
        with requests_mock.Mocker() as m:
            m.get(self.mock_url, status_code=404)
            self.assertEqual(site_dict(self.sites, self.mock_prefix), None)
